...
{#if song}
  <div class="container mx-auto max-w-2xl p-2">
    <div class="bg-gray-950 rounded-lg shadow-lg overflow-hidden mb-2">
      <h1
        class="text-xl font-bold text-white p-4 text-center cursor-pointer hover:bg-gray-900 transition-colors" 
    ...
    <div class="flex justify-between items-center mb-4">
      <div class="flex flex-col">
        <p class="text-gray-300 text-sm">
          Original Key: <span class="text-[#d28fff] font-medium"
            >{originalKey}</span
          >
        </p>
        <p class="text-gray-300 text-sm">
          Current Key: <span class="text-[#d28fff] font-medium"
            >{currentKey}</span
          >
        </p>
      </div>
      ...
      <div class="flex items-center gap-2">
        <button
          ...
          class="bg-gray-950 hover:bg-gray-900 p-2 rounded-full transition-colors"
    ...
    <div
      class="bg-gray-950 rounded-lg p-3 mb-6 flex items-center justify-between"
    >
      <div class="flex items-center gap-2">
        <button
          on:click={zoomOut}
          class="bg-gray-900 hover:bg-gray-800 text-white px-3 py-1 rounded-md transition-colors"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            class="h-5 w-5"
        ...
        <span class="text-gray-300 text-sm">{Math.round(fontSize * 100)}%</span>
      ...
      <button
        ...
        class="bg-[#7623ad] hover:bg-[#5a1b85] text-white px-4 py-1 rounded-md transition-colors flex items-center gap-2"
      >
        <svg
          xmlns="http://www.w3.org/2000/svg"
          class="h-5 w-5"
    ...
    <div class="lyrics-container" style="font-size: {fontSize}rem;">
      <pre
        class="lyrics bg-gray-950 text-white p-4 rounded-lg shadow-lg mb-20">{transposedLyrics}</pre>
    </div>
  </div>

  {#if showModal}
    <div 
      class="fixed inset-0 bg-black/50 bg-opacity-80 flex items-center justify-center z-50" 
      on:click={toggleModal}>
      <div 
      class="bg-gray-900 rounded-lg shadow-xl p-6 max-w-md w-full mx-4" 
      ...
      <div class="flex justify-between items-center mb-4">
        ...
        on:click={toggleModal}>&times;</button>
      </div>

      <p class="text-gray-300 mb-4">
        Artist: <span class="text-[#bf5eff]">{song.artist || "Unknown Artist"}</span>
      </p>
      <h3 class="text-lg font-semibold text-white mb-4">YouTube Video:</h3>

      {#if embedUrls.length > 0}
        <div class="grid gap-4">
        {#each embedUrls as url}
          <div class="relative w-full pb-[56.25%] h-0 rounded-lg overflow-hidden shadow-lg">
          <iframe
    ...
    >
      ...
      >
        <div class="flex justify-between items-center mb-4">
          ...
          <button
            class="text-gray-400 hover:text-white text-2xl"
            on:click={closePlaylistModal}>&times;</button
          >
        </div>
        ...
        {#if userId}
          {#if playlists && playlists.length > 0}
            <div class="max-h-60 overflow-y-auto mb-4">
              {#each playlists as playlist}
                <div
                  class="flex items-center p-3 border-b border-gray-700 hover:bg-gray-700 transition-colors"
                >
                  <input
                    type="radio"
                    id={playlist.id}
                    ...
                    class="mr-3"
                  />
                  <label for={playlist.id} class="text-white cursor-pointer"
            ...
            <div class="flex justify-end gap-3">
              <button
                ...
                class="bg-[#7623ad] hover:bg-[#5a1b85] text-white px-4 py-2 rounded-md transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              ...
              <button
                on:click={closePlaylistModal}
                class="bg-gray-950 hover:bg-black text-white px-4 py-2 rounded-md transition-colors"
              >
                Cancel
              </button>
            </div>
          {:else}
            ...
            <div class="flex justify-end gap-3">
              <a
                href="#"
                class="bg-[#7623ad] hover:bg-[#5a1b85] text-white px-4 py-2 rounded-md transition-colors"
                on:click={(e) => {
...
{:else}
  <div class="container mx-auto max-w-2xl p-4">
    <div class="bg-gray-800 rounded-lg shadow-lg p-6 text-center">
      <p class="text-red-400 text-lg">Song not found. Please check the URL.</p>
    </div>
  </div>
{/if}
